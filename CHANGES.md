...
## [Task 10] Monthly Balance Estimation (with Mixed-Currency Support)

- **Date**: 2026-08-28
- **Technical Summary**: Implemented upcoming month balance estimation tool projecting dual-currency balances based on recurring monthly income and expenses, dynamically linked to live balances with mixed-currency (USD + LBP) item support, month-scoped skip, and temporary edit overrides.

### Technical Log

- **Modified**: `src/data/storage.js` — Added `KEYS.MONTHLY_ESTIMATES`, `getUpcomingMonthKey()`, `getMonthlyEstimates()`, `addMonthlyEstimate()`, `updateMonthlyEstimate()`, `deleteMonthlyEstimate()`, `toggleSkipMonthlyEstimate()`, `setMonthlyEstimateTempOverride()`, `clearMonthlyEstimateTempOverride()`, and `getEstimatedBalances()`. Supported `amounts: [{ amount, currency }]` across CRUD operations and temporary overrides with backwards-compatible fallbacks. Updated `exportData()` and `importData()` to include `monthlyEstimates` in backups and triggered auto-backups on mutations.
- **Modified**: `src/index.html` — Added secondary schedule FAB button (`#btn-open-estimate`) stacked vertically above the `+` FAB, and added `<dialog id="estimate-dialog" class="full-dialog">` with summary cards, dynamic multi-currency amount rows (`#estimate-amounts-rows`), and scrollable recurring items list.
- **New**: `src/ui/estimation.js` — Created estimation UI controller module handling dialog lifecycle, dynamic row toggling for mixed USD/LBP amounts, dynamic projection updates, month-scoped Skip/Unskip, Temp Edit override/undo, permanent Edit, and Delete actions.
...
- **New**: `test/estimation-check.js` — Added pure assert self-check testing key rollovers, mixed-currency item projections, live balance reactivity, month-scoped skip, and temp overrides.
- **Why**: Allows users to forecast next month's cash flow in both USD and LBP based on recurring bills and income without modifying actual ledger entries, while supporting single or mixed-currency bills (e.g., $10 + 500,000 LBP).

### Plain English Summary

You can now tap the schedule (📅) button above the "+" button to open the Monthly Estimation screen. Here, you can add your recurring monthly income and bills in USD, LBP, or mixed currencies (e.g. $10 + 500,000 LBP in a single item). The app automatically projects your wallet balances for next month by taking your real-time cash balance and applying your upcoming monthly net totals. You can temporarily skip any bill for the upcoming month, apply a one-off temporary amount change for next month only, permanently edit recurring amounts, or delete items anytime.
